...
export async function middleware(req) {
    ...
    // بررسی userAgent
     if(_userAgent(req))return new Response(isBotMessage)
    ...
    if (req.nextUrl.pathname.startsWith('/api/dashboard')) {
        ...
        return response;
    } else if (req.nextUrl.pathname.startsWith('/api/profile') || req.nextUrl.pathname.startsWith('/api/seller')) {
...
function isBotMessage(){
    return new Blob(['<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0"><title>boot</title></head><body dir="rtl" style="height:100vh;width:100vw;background-color:#0a0010; overflow:hidden; display:flex; flex-direction:column; align-items: center " ><h2 style="text-align:center; margin-inline:auto;color:#a22; margin-top:30px" >اگر فیلتر شکن شما روشن هست آن را خاموش کنید</h2></body></html>']),
