...
import { TicketModel } from '@/models/Tikets'; // وارد کردن مدل تیکت‌ها
...
export async function GET(req) {
    try {
        ...
        return Response.json(tickets); // بازگرداندن تیکت‌ها به صورت JSON
    } catch (error) {
        return Response.json(error?.message, { status: (error && error.status) || 500 }); // بازگرداندن خطا در صورت وقوع
    }
}