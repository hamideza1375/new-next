...
export async function PUT(req: NextRequest, { params }: { params: Params }): Promise<NextResponse> {
    try {
        ...
        if (video?.size) {
            ...
            // حذف فایل قدیمی
            if (part.videoUrl && existsSync(path.join(process.cwd(), 'assets/uploads/product/', part.videoUrl))) {
                unlinkSync(path.join(process.cwd(), 'assets/uploads/product/', part.videoUrl));
            }
        }
        ...
        if (source?.name) {
            ...
            if (
                part.sourceUrl &&
                existsSync(path.join(process.cwd(), 'assets/uploads/product/', part.sourceUrl))
            ) {
                unlinkSync(path.join(process.cwd(), 'assets/uploads/product/', part.sourceUrl));
            }
        }
        ...
        await ProductsModel.updateOne(
            { _id: params.id[0], 'parts._id': params.id[1] },
            {
                $set: {
                    ...
                    ...(videoFilename && { 'parts.$.videoUrl': videoFilename }),
                    ...(sourceFilename && { 'parts.$.sourceUrl': sourceFilename })
                }
            }
        );
...
export async function DELETE(req: NextRequest, { params }: { params: Params }): Promise<NextResponse> {
    try {
        ...
        if (part.sourceUrl && existsSync(path.join(process.cwd(), 'assets/uploads/product/', part.sourceUrl))) {
            unlinkSync(path.join(process.cwd(), 'assets/uploads/product/', part.sourceUrl));
        }
